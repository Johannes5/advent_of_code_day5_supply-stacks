...
function moveCrates( amount: number, from: number, to: number) {
        // depending on the amount of crates to move, slice the last x crates from the from stack
        ...
        console.log("stack with new crates", stacks[to-1]);
}
...
rearrangementSteps.forEach(step => {
    ...
    moveCrates(numberOfCrates, from, to);
})
